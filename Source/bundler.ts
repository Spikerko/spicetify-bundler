import {
  defaultExternalGlobalPluginExternals,
  externalGlobalPlugin,
} from "./EsbuildPlugins/externalGlobalPlugin.ts";
...
const WriteTextFile = (path: string, contents: string): Promise<void> => {
  return Deno.mkdir(dirname(path), { recursive: true }).then((_) =>
...
export function resolve(from, to) {
        ...
        setup(build) {
          ...
          build.onLoad(
            ...
            async (args) => {
              ...
              const processed = await postCSSProcessor.process(result.css, {
                from: args.path,
              });
              rawCSS.push(
                FormatCSSFile(
                  relative(absoluteSourcePath, args.path),
                  processed.css
                )
              );
        ...
        setup(build) {
          ...
          build.onLoad(
            ...
            async (args) => {
              ...
              const processed = await postCSSProcessor.process(contents, {
                from: args.path,
              });
              rawCSS.push(
                FormatCSSFile(
  ...
  plugins.push(
    (CustomBuildOptions?.skipGlobalReplacementRules
      ? externalGlobalPlugin(
          CustomBuildOptions?.additionalGlobalReplacementRules ?? {}
        )
      : externalGlobalPlugin({
          ...defaultExternalGlobalPluginExternals,
          ...(CustomBuildOptions?.additionalGlobalReplacementRules ?? {}),
  ...
  const buildDir =
    BuildDir !== undefined && BuildDir !== "" ? BuildDir : "./dist";
  const buildDirectory =
    Type === "Release" ? join(Deno.cwd(), buildDir) : undefined;
